...
Если принцип модулей переиспользование кода. То в данном случае у нас, к примеру, имеется структура приложение-бд. Таким образом можем применять модули для данной архитектуры приложения.
Значит для модулей app, db параметрзируем дополнительно следующие параметры:
...
теперь можно использовать код для любого приложения по данной архитектуре. app-db(MongoDB, PostgreSQL, MySQL и т.д.)
можно сказать инкапсулировали реализацию и оставили только архитектуру для использования (имею в виду интерфейс, в котором необходимо ввести параметры)
